...
@task
def test_timestamp():
    print('ccn_{}'.format(make_timestamp()))
...
def make_backup():
    with cd('/product/apache2/'):
        sudo('tar -zcvf ccn_{}_test.tar.gz ccn/'.format(make_timestamp()))
...
def fix_permissions():
    with cd('/product/apache2/'):
        sudo('chown -R custapache:apps ccn/')


@task
def copy_app():
    with cd('/product/apache2/ccn/'):
        sudo('rm -rf /product/apache2/ccn/*')
